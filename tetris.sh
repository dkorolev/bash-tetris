...
#
# Translate: Rojen Zaman | https://github.com/rojenzaman/bash-tetris-russian
...
update_score() {
    ...
    set_bold
    set_fg $SCORE_COLOR
    xyprint $SCORE_X $SCORE_Y         "ПОЛНЫХ СТРОК: $lines_completed"
    xyprint $SCORE_X $((SCORE_Y + 1)) "УРОВЕНЬ:      $level"
    xyprint $SCORE_X $((SCORE_Y + 2)) "СЧЕТ:         $score"
    reset_colors
}
...
"      Q: УВОЛИТЬСЯ"
"  C: ПЕРЕКЛЮЧИ́ТЬ ЦВЕТ"
"N: ПЕРЕКЛЮЧИ́ТЬ ПОКАЗА́ТЬ ДА́ЛЬШЕ"
"H: ПЕРЕКЛЮЧИТЬ СЛЕДУЮЩИЙ"
)
